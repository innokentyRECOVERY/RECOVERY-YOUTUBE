# RECOVERY YOUTUBE

Обход блокировки YouTube и Telegram через VLESS-подписку (split-tunnel: через VPN идут только сайты нужного сервиса, остальной интернет не затрагивается).

Unblock YouTube and Telegram via VLESS subscription (split-tunnel: only the chosen service's traffic goes through VPN, the rest of the Internet is unaffected).

## Быстрый старт (Windows)

1. Скачайте `RECOVERY_YOUTUBE.exe` из [Releases](https://github.com/innokentyRECOVERY/RECOVERY-YOUTUBE/releases).
2. Запустите его, выберите вкладку вверху («ОБХОД YOUTUBE» или «ОБХОД TELEGRAM») и нажмите «ЗАПУСТИТЬ ОБХОД».
3. При первом запуске автоматически скачается `xray-core` (нужен доступ к GitHub).
4. Откройте YouTube или `https://web.telegram.org/` (Telegram Desktop) — будет работать.

## Установка на телефон (Android)

1. Скачайте `RECOVERY_YOUTUBE.apk` из [Releases](https://github.com/innokentyRECOVERY/RECOVERY-YOUTUBE/releases) (актуальная версия **v2.0.0.4**).
2. Установите APK (разрешите установку из неизвестных источников).
3. Откройте приложение и нажмите кружок с символом питания — при первом запуске Android спросит разрешение на VPN, нажмите «ОК».
4. Через VPN пойдут только YouTube и Telegram, остальной интернет — напрямую. Лимит сессии — 2 часа.

## Что делает программа

- Две вкладки: «ОБХОД YOUTUBE» и «ОБХОД TELEGRAM».
- Подключается к серверу Англия из VLESS-подписки.
- Split-tunnel: через VPN идут только домены выбранного сервиса — YouTube (`youtube.com`, `googlevideo.com`, `ytimg.com`, `youtu.be` и др.) либо Telegram (`telegram.org`, `web.telegram.org`, `t.me`, `cdn.telegram.org` и др.), весь остальной трафик — напрямую.
- Показывает реальную скорость трафика (⬆ отдаёт / ⬇ получает).
- Лимит сессии 2 часа с автоотключением, повторный запуск — вручную.
- Поддержка смены кода подписки без перезапуска.

## Как это работает

- GUI на `tkinter` (Python).
- Ядро — [Xray-core](https://github.com/XTLS/Xray-core), скачивается автоматически.
- Системный прокси WinINET ставится на `127.0.0.1:10809`, только для процессов, использующих системный прокси.

## Сборка из исходников

Исходный код не публикуется.

## Важно

- Подписка Не нуждается в оплате (если кто то хочет свой Сервер вводите в поле «Новый код подписки»).
- Кнопка «закрыть» (X) сворачивает приложение в системный трей — обход продолжает работать.
- Полный выход — через значок в трее (меню «Выход»). При выходе системный прокси снимается автоматически.

## История версий

| Версия | Что изменилось |
|--------|----------------|
| **v2.0.0.4** | Android 1.0.2.5: исправлен повторный запрос разрешения VPN после первого подтверждения (выдавался второй системный диалог не открываясь, соединение не поднималось). Защита от двойного `prepare()`, пошаговые статусы запуска и лог этапов в «Прошлом сбое» для точной диагностики. versionCode 5. |
| **v2.0.0.3** | Android 1.0.2.4: исправлен вылет сразу при нажатии «включить» на Android 14+ (`MissingForegroundServiceTypeException`) — добавлен `foregroundServiceType="dataSync"` и permission `FOREGROUND_SERVICE_DATA_SYNC`, иконка уведомления перенесена на отдельный drawable, `startForeground` обёрнут в try/catch. versionCode 4. |
| **v2.0.0.2** | ПК-сборка 1.0.2.4: кнопка питания теперь использует точные изображения круглых кнопок из макета (красный кружок = выкл, зелёный = вкл), кнопка крупнее и с более тёмной обводкой. Одна кнопка-переключатель вместо двух («ЗАПУСТИТЬ» + «ОСТАНОВИТЬ»). |
| **v2.0.0.1** | Исправление Android-сборки: убран `BIND_VPN_SERVICE` permission (краш при нажатии кружка), `tun2socks` пересобран под android/arm64 (старый был glibc-бинарь и не мог работать на Android), добавлен перехватчик крашей с показом ошибки на экране. |
| **v2.0.0.0** | Выход мобильного приложения: `RECOVERY_YOUTUBE.apk` для Android. Тот же интерфейс (вкладки «ОБХОД YOUTUBE»/«ОБХОД TELEGRAM», круглая кнопка «ЗАПУСТИТЬ», статус, сервер Англия), те же домены и подписка. Работает как VPN: через туннель идут только IP YouTube и Telegram, остальной интернет — напрямую. Внутри приложения остаётся версия 1.0.2.3 — мобильная сборка того же десктопного релиза. |
| **v1.0.2.3** | Кнопка «ЗАПУСТИТЬ» стала круглой: вместо текста — символ питания (круг с палочкой), подпись под кнопкой. Фон окна встроен прямо в exe — отдельный файл `spokoynich.jpg` больше не нужен, картинка загружается из самой программы. Кнопка смещена вниз, чтобы не наезжать на текст сервера. |
| **v1.0.2.2** | Исправлена сборка: библиотека tkinter и зависимости TCL/TK теперь корректно включаются в exe-файл. Устранена проблема, из-за которой PyInstaller выбрасывал модули графического интерфейса, оставляя пустую рамку окна при запуске. Исправлен мелкий баг предыдущей сборки. Возможности те же, что и в v1.0.2.1.
|**v1.0.2.1** | Исправлена сборка: tkinter теперь корректно включается в exe (раньше из-за повреждённой переменной окружения TCL_LIBRARY программа падала при запуске с ошибкой «No module named 'tkinter'»). Возможности те же, что в v1.0.2.0. |
| **v1.0.2.0** | Добавлена поддержка Telegram: две вкладки вверху окна — «ОБХОД YOUTUBE» и «ОБХОД TELEGRAM». Через VPN идут домены Telegram (`telegram.org`, `web.telegram.org`, `t.me`, `cdn.telegram.org`) — работает и в браузере (`web.telegram.org`), и в Telegram Desktop. Сервер и подписка те же. |
| **v1.0.1.0** | Кнопка «📁 Выбрать папку для xray» стала растягиваться по длине текста — надпись больше не обрезается. |
| **v1.0.0.9** | Защита от повтора: теперь обнаруживается уже запущенный экземпляр любой версии (включая старые сборки без mutex, у которых окно может быть свёрнуто в трей) — новый запуск поднимает существующее окно и не создаёт второе. |
| **v1.0.0.8** | Окно выше (560×650), кнопки и отступы уплотнены — поле кода подписки и кнопка «ПРИМЕНИТЬ КОД» больше не обрезаются. Код применяется по клавише Enter. |
| **v1.0.0.7** | Собственная иконка встроена прямо в код (PNG base64) — теперь она показывается и в системном трее, и в панели задач вместо стандартной иконки Python/tk. Больше не зависит от файла иконки на диске. |
| **v1.0.0.6** | Контроль реальной доступности сервера: если соединение крутится бесконечно, а YouTube через прокси не открывается — программа показывает предупреждение «сервер отключён / подписка закончилась (оплата/истекла) / возможен DDoS» и прекращает подключение. |
| **v1.0.0.5** | Кнопка «ОСТАНОВИТЬ» отключает от сервера, но программа остаётся открытой (не закрывается). Приложение ограничено одним экземпляром: повторный запуск поднимает уже открытое окно (в т.ч. из трея), а не создаёт новое — сервер не нагружается. |
| **v1.0.0.4** | Если при нажатии X соединение активно — приложение сворачивается в трей и следит за связью с сервером; когда соединение отключится, программа закрывается сама. Если при X соединения нет — закрывается сразу. |
| **v1.0.0.3** | Кнопка «закрыть» больше не выключает обход — приложение сворачивается в системный трей и продолжает работать. Полное завершение — через значок в трее («Выход»). |
| **v1.0.0.2** | Создаётся своя папка `%APPDATA%\RECOVERY_YOUTUBE` — все файлы хранятся там. Xray-core скачивается один раз и не перекачивается; старый xray переносится из папки exe автоматически. |
| **v1.0.0.1** | Версия в заголовке окна и метаданных exe; диалог выбора папки показывает скрытые файлы. |
| **v1.0.0** | Первый выпуск: подключение к серверу Англия, split-tunnel только для YouTube, лимит сессии 2 часа, счётчик трафика, автоскачивание xray-core, выбор папки загрузок. |
